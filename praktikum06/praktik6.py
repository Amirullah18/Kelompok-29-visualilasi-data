...
ax.set_title('Sales Transactio by Store')
ax.set_xlabel('Stores')
ax.set_ylabel('Sales')
ax.set_xticklabels(stores)
ax.set_xticks(x)
...
# 3 Graksik Kustomisasi Stacked Vertical Bar Chart
...
for i in range(len(x)):
    ...
    plt.text(x[i], product_b[i] + product_b[i]/2, str(product_b[i]), ha='center', color='black')
st.pyplot(fig)
